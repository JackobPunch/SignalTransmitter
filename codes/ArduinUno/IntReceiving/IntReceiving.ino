...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (value >= 0 && value <= 4095) { // Ensure value is within DAC range
      mcp.setChannelValue(MCP4728_CHANNEL_A, value*2); // Set value to channel A
      }/*// Blink the LED on pin 13
      digitalWrite(13, HIGH);
      delay(100); // Keep the LED on for 100 milliseconds
      digitalWrite(13, LOW);
      //Serial.println("Value set to DAC: " + String(value*2));
    } else {
      Serial.println("Invalid value: " + String(value*2) + ". Value must be between 0 and 4095.");
    }*/
  }
}