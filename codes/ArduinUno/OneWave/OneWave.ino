#include <Adafruit_MCP4728.h>
#include <Wire.h>

Adafruit_MCP4728 mcp;
int values[]={366,366,388,398,402,419,471,509,607,643,741,797,915,979,1127,1197,1366,1443,1550,1600,1696,1738,1793,1824,1869,1896,1900,1904,1925,1923,1916,1926,1948,1947,1957,1973,1987,2005,2041,2047,2064,2068,2078,2076,2065,2041,1967,1924,1829,1765,1639,1568,1429,1351,1179,1099,952,890,785,734,649,628,540,502,460,432,408,405,411,430,432,412,401,405,396,398,388,385,371,371,371,366,366,388,398,402,419,471,509,607,643,741,797,915,979,1127,1197,1366,1443,1550,1600,1696,1738,1793,1824,1869,1896,1900,1904,1925,1923,1916,1926,1948,1947,1957,1973,1987,2005,2041,2047,2064,2068,2078,2076,2065,2041,1967,1924,1829,1765,1639,1568,1429,1351,1179,1099,952,890,785,734,649,628,540,502,460,432,408,405,411,430,432,412,401,405,396,398,388,385};

void setup() {
  Serial.begin(9600);

  // Try to initialize!
  if (!mcp.begin()) {
    Serial.println("Failed to find MCP4728 chip");
    while (1) {
      delay(10);
    }
  }
}

void loop() {
  int arraySize=sizeof(values)/sizeof(values[0]);
  for(int i = 0; i < arraySize; i++) {
    mcp.setChannelValue(MCP4728_CHANNEL_A, values[i]); 
    delayMicroseconds(150);
    if(i >= 4 && i < arraySize - 4) { // Stop generating signal for Channel B once Channel A is done
      mcp.setChannelValue(MCP4728_CHANNEL_B, values[i-4]);
    }
  }
  delay(2000);
}